...
Plex::Plex(::S3RequestContext * requestContext)
...
void Plex::initialize() {
	...
	logger.debug("Plex@%p: requests=%p\n", this, requests);
}
...
int Plex::complete() {
	...
	if (status != S3StatusOK) {
		Logger::instance().error("Plex@%p: S3_runall_request_context failed! status=%d=\"%s\"\n", this, status, tostring(status));
		rc = (::S3_status_is_retryable(status) != 0) ? RETRY : ERROR;
	}
	return rc;
}

int Plex::iterate(int & pending /* static dontcare */) {
	Logger & logger = Logger::instance();
	int rc = 0;
	int count = 0;
	::S3Status status = ::S3_runonce_request_context(requests, &count);
	if (status != S3StatusOK) {
		logger.error("Plex@%p: S3_runonce_request_context failed! status=%d=\"%s\"\n", this, status, tostring(status));
		rc |= (::S3_status_is_retryable(status) != 0) ? RETRY : ERROR;
	}
	if (count > 0) {
		logger.debug("Plex@%p: pending=%d\n", this, count);
		rc |= PENDING;
	}
	pending = count;
	return rc;
...
int Plex::ready(Milliseconds timeout) {
	Logger & logger = Logger::instance();
	int rc = 0;
	do {
		...
		if (status != ::S3StatusOK) {
			logger.error("Plex@%p: S3_get_request_context_fdsets failed! status=%d=\"%s\"\n", this, status, tostring(status));
			rc |= (::S3_status_is_retryable(status) != 0) ? RETRY : ERROR;
			break;
		}
		logger.debug("Plex@%p: maxfd=%d\n", this, maxfd);
		if (maxfd < 0) {
			break;
		}
		...
		logger.debug("Plex@%p: milliseconds=%lld\n", this, timeout);
		...
		if (::select(maxfd, &reads, 0, 0, &timeoutval) < 0) {
			logger.error("Plex@%p: select failed! error=%d=\"%s\"\n", this, errno, ::strerror(errno));
			rc |= ERROR;
			break;
		}
	...
	logger.debug("Plex@%p: ready=0x%x\n", this, rc);
	return rc;
}

int Plex::service(Milliseconds timeout, int limit) {
	...
	Logger::instance().debug("Plex@%p: service=0x%x\n", this, rc);
	return rc;
}
